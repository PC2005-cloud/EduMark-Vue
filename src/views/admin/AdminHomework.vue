...
const filtered = computed(() => {
  ...
  if (filterUserId.value) {
    items = items.filter(t => t.user_id === filterUserId.value)
  }
  return items
})
...
const allRecords = ref<TaskOut[]>([])

async function load() {
  loading.value = true
  try {
    const res = await homeworkApi.pageTask({ page_num: 1, page_size: 100, sort_fields: [{ field: 'create_time', direction: 'desc' }] })
...
async function loadUsers() {
  try {
    ...
    res.rows.forEach(u => {
      map[u.id] = u.username
      // 作业仅学生提交
      if (u.role === 'student') list.push({ id: u.id, name: u.username })
    })
    userMap.value = map
...
<template>
  <div class="animate-slide-up">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
      <div>
        <h2 class="text-2xl lg:text-3xl font-heading font-bold text-ink tracking-tight">作业预览</h2>
        <p class="text-ink-secondary mt-1">查看所有用户的作业提交与批改结果</p>
      </div>
    </div>
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-surface-muted">
            <tr v-for="task in records" :key="task.task_id" class="hover:bg-surface-secondary transition-colors">
              ...
              <td class="px-6 py-4 text-sm text-ink">{{ userMap[task.user_id] || `用户#${task.user_id}` }}</td>
